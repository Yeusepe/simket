/**
 * Purpose: Manage editable Framely page schemas with selection, ordering, and theme updates.
 * Governing docs:
...
import { createPageSchema, type PageBlock, type PageSchema, type ThemeOverrides } from './types.js';
...
function removeBlock(blocks: readonly PageBlock[], id: string): readonly PageBlock[] {
  return blocks
...
function moveBlock(blocks: readonly PageBlock[], id: string, offset: number): readonly PageBlock[] {
...
export function useBuilder(initialSchema?: PageSchema) {
  const [schema, setSchema] = useState<PageSchema>(() => createPageSchema(initialSchema));
