/**
 * Purpose: Define JSON-serializable page schema contracts for Framely-rendered pages.
 * Governing docs:
...
export interface FramelyRenderContext {
  readonly kind: 'page' | 'product' | 'store' | 'preview';
  readonly product?: Record<string, unknown>;
  readonly store?: Record<string, unknown>;
  readonly hrefs?: Record<string, unknown>;
}
...
export function createPageSchema(overrides: Partial<PageSchema> = {}): PageSchema {
  return {
...
function validatePageBlock(block: unknown, path: string): readonly string[] {
  if (!isRecord(block)) {
...
export function validatePageSchema(schema: unknown): PageSchemaValidationResult {
  if (!isRecord(schema)) {
    return {
      success: false,
